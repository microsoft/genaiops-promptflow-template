...
def prepare_and_execute(
    ...
    if EXECUTION_TYPE == "LOCAL":
        ...
        ml_client = AIClient(
            subscription_id="f5772c02-4566-4154-a8d5-5b4658e5d310",
            resource_group_name="stai",
            project_name="rimod-0022",
            ...
            for variant in flow_detail.all_variants:
                for variant_id, node_id in variant.items():
                    ...
                    if not check_dictionary_contained(
                        ...
                        #run = pf.runs.create_or_update(run1, stream=True)
            ...
            if (flow_type == FlowTypeOption.DAG_FLOW or
                ...
                run = pf.run(
                    ...
                    data=(
                        dataset.get_local_source(base_path)
                        if EXECUTION_TYPE == "LOCAL"
                        else dataset.get_remote_source(ml_client._ml_client)
                    ),
                    name=run_name,
                    display_name=run_name,
                    environment_variables=env_vars,
