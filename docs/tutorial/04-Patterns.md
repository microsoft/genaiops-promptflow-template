...
- Flows can be executed using dedicated runtime and compute instance in Azure ML from CI/CD pipelines. This is the default behavior of the [GitHub Actions](../../.github/workflows/) in this repo and the runtime need to be configured in the `llmops_config.yaml` file in the configs folder under root of the specific use case folder. Example: for [web_classification](../../web_classification/) use case runtime is configured in [llmops_config.yaml](../../web_classification/configs/llmops_config.yaml) file.
